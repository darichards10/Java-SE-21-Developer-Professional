...
/**
 * EXAM TOPIC: Handling Exceptions
 * Oracle Java SE 21 Developer Professional (1Z0-830) — ~7% of exam
 *
 * Covers:
 * - Checked vs unchecked exceptions:
 *     Checked (extends Exception, not RuntimeException) — must be declared with throws or caught
 *     Unchecked (extends RuntimeException or Error) — no compile-time enforcement
 * - try-catch-finally blocks: finally always executes (even after return or exception)
 * - Multi-catch (catch (A | B e)): catches multiple exception types in one block;
 *     the variable is implicitly final; types must not be related (no supertype/subtype relationship)
 * - try-with-resources (AutoCloseable): resource declared in try(...) is automatically closed;
 *     close() is called before any catch or finally block; resources closed in reverse declaration order
 * - Suppressed exceptions: if body and close() both throw, body exception is primary;
 *     close() exception added as suppressed via getSuppressed()
 * - Custom exception classes: extend Exception (checked) or RuntimeException (unchecked)
 * - Exception chaining (cause): new Exception("msg", cause) — retrieve with getCause()
 * - Common exam exceptions: NullPointerException, ArrayIndexOutOfBoundsException,
 *     ClassCastException, NumberFormatException, ArithmeticException (integer /0),
 *     StackOverflowError, IllegalArgumentException, IllegalStateException
 */
public class ExceptionExamples {
	...
	static void tryWithResources() {
		// EXAM: AutoCloseable resources are closed before catch/finally; reverse declaration order

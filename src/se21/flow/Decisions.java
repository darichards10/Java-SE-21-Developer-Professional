...
/**
 * EXAM TOPIC: Controlling Program Flow Oracle Java SE 21 Developer Professional
 * (1Z0-830) — ~9% of exam
 *
 * Covers: - Pattern matching for instanceof (Java 16+): combines type check and
 * cast in one step - Traditional switch statement vs switch expression (Java
 * 14+) - Switch expression with arrow labels (->): no fall-through, no break
 * needed - Switch expression with yield: used inside block bodies to return a
 * value - Pattern matching for switch (Java 21): case Integer i, case Double d,
 * guarded patterns (when) - Null case in switch (Java 21): case null handles
 * null without NullPointerException - do-while loop: body executes at least
 * once before condition is checked - Labeled break/continue: exits or skips to
 * a specific outer loop
 */
public class Decisions {

	static void printIntegersGreaterThan5(Number number) {
		// EXAM: pattern matching instanceof — check AND cast in one statement; 'data'
		// scoped to true-branch
	...
	static String getAnimalBetter(int type) {
		String animal;
		switch (type) { // EXAM: traditional switch statement — requires break; fall-through is a common
						// exam trap
		case 0:
			animal = "Lion";
			break;
	...
	static String getAnimalBest(int type) {
		return switch (type) { // EXAM: switch expression with arrow labels — no fall-through, no break,
								// returns a value
		case 0 -> "Lion";
		case 1 -> "Elephant";
		case 2, 3 -> "Alligator";
	...
	static void getTrainer(Number height) {
		...
		case Integer i when i > 10 -> "Joseph"; // EXAM: guarded pattern (when) — Java 21; condition refines the type
												// match
	...
	public static void doWhile() {
		int lizard = 0;
		do {
			lizard++; // EXAM: do-while executes body BEFORE checking condition; lizard is always at
						// least 1
		} while (false);
	...
	static void breakLoop() {
		...
		PARENT_LOOP: for (int i = 0; i < list.length; i++) { // EXAM: labeled loop — label names the loop to target with
																// break/continue
